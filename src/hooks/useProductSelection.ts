...
export function useProductSelection(): UseProductSelectionReturn {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ Successfully fetched products:', data.products.length)
  ...
  const clearSelection = useCallback(() => {
    console.log('🗑️ Cleared product selection')
  ...
  const convertImageUrlToFile = useCallback(async (imageUrl: string, filename: string): Promise<File | null> => {
    try {
      ...
      showError(`Failed to load product image: ${errorMessage}`)
      return null
    }
  }, [showError])
